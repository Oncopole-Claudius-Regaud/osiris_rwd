...
from extract_familycancerhistory_by_ipp import (
    extract_hits_for_document,
    family_scope,
    is_family_source_document,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Debug familyCancerHistory regex matches for one IPP.")
    ...
    try:
        for metadata_path in sorted(source_dir.glob("*.json.txt")):
            ...
            if metadata_ipp(metadata, metadata_path) != args.ipp:
                continue
            docs_seen += 1
            selected = is_family_source_document(metadata)
            if not args.all_documents and not selected:
                continue

            pdf_path = metadata_pdf_path(metadata_path)
            print(f"\nDOCUMENT | family_source={selected} | date={metadata_date(metadata)} | pdf={pdf_path.name}")
            ...
            scope = family_scope(text)
            if args.show_scope:
                print(f"  scope={compact(scope) if scope else '<NO_FAMILY_SECTION>'}")
            ...
            for hit in hits:
                ...
                print(
                    "  MATCH | "
                    f"topo={hit.familycancertopocode} | parentage={hit.familycancerparentage} | "
        ...
        print(
            f"\nSUMMARY | ipp={args.ipp} | docs_seen={docs_seen} | "
            f"docs_selected={docs_selected} | hits={hits_total}"
        )
    finally:
        if output_handle:
            output_handle.close()
